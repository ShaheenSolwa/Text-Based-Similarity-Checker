...
file1 = st.file_uploader('Upload File 1', type=['txt', 'pdf'], accept_multiple_files=True)
file2 = st.file_uploader('Upload File 2', type=['txt', 'pdf'], accept_multiple_files=True)
...
if st.button('Calculate Similarity'):
    if file1 and file2:
        texts1 = read_file(file1)
        texts2 = read_file(file2)
        ...
        for i, text1 in enumerate(texts1):
            for j, text2 in enumerate(texts2):
                ...
                data.append({'Similarity Measure': similarity_measure, 'File 1': file1[i].name, 'File 2': file2[j].name,
